...
deploy()
{
	...
	echo ""
	echo "deploy..."
	if ! ssh -p $serverPort -i ~/ssh-keys/$serverKey.pem mms@$serverAddress "[ -f '/opt/catramms/CatraLibraries/scripts/deploy.sh' ]"; then
		ssh -p $serverPort -i ~/ssh-keys/$serverKey.pem mms@$serverAddress "tar xfz /opt/catramms/$tarFileName -C /opt/catramms"
		ssh -p $serverPort -i ~/ssh-keys/$serverKey.pem mms@$serverAddress "ln -s /opt/catramms/$moduleName-$version /opt/catramms/CatraLibraries"
	fi
	ssh -p $serverPort -i ~/ssh-keys/$serverKey.pem mms@$serverAddress "/opt/catramms/CatraLibraries/scripts/deploy.sh $version"
