...
public:
  MySQLConnectionFactory(string dbServer, string dbUsername, string dbPassword,
                         string dbName, bool reconnect,
  ...
  shared_ptr<DBConnection> create(int connectionId) {
    try {
      ...
      SPDLOG_DEBUG("sql connection creating..."
                   ...
                   _dbServer, _dbUsername, _dbPassword, _dbName, _reconnect,
                   _defaultCharacterSet);
#endif

      // server like "tcp://127.0.0.1:3306"
      shared_ptr<sql::Connection> connectionFromDriver(
          driver->connect(connection_properties));

      // shared_ptr<sql::Connection> connectionFromDriver
      // (driver->connect(_dbServer, _dbUsername, _dbPassword)); bool
      // reconnect_state = true;
      // connectionFromDriver->setClientOption("OPT_RECONNECT",
      // &reconnect_state); connectionFromDriver->setSchema(_dbName);

      shared_ptr<MySQLConnection> mySqlConnection =
          make_shared<MySQLConnection>(_selectTestingConnection, connectionId);
      ...
      if (!connectionValid) {
        ...
        SPDLOG_DEBUG("just created sql connection"
                     ...
                     ", _dbUsername: {}"
                     ", _dbName: {}",
                     mySqlConnection->getConnectionId(), _dbServer, _dbUsername,
                     _dbName);
#endif
      }
