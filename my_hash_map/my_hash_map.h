...
inline int GENERIC_intHASH(const int dataToHash) {
  return ( (int) ( g_HASH_SEED * ( g_HASH_CONSTANT * ( dataToHash) - 
				   (int) (g_HASH_CONSTANT*( dataToHash) ))));
}

#ifdef WIN32
	inline __int64 GENERIC___int64HASH(const __int64 dataToHash) {
	return ( (__int64) ( g_HASH_SEED * ( g_HASH_CONSTANT * ( dataToHash) - 
					(__int64) (g_HASH_CONSTANT*( dataToHash) ))));
	}
#else
	inline long long GENERIC_longlongHASH(const long long dataToHash) {
	return ( (long long) ( g_HASH_SEED * ( g_HASH_CONSTANT * ( dataToHash) - 
					(long long) (g_HASH_CONSTANT*( dataToHash) ))));
	}
#endif
...
typedef struct BufferHasher: MyHasherModel<Buffer_p>
{
	public:
		...
		{
			int i, tempResult, result = 0;
			const int length = strlen (
				(const char *) (*key));

			for(i=0; i<length ; i++)
			{
				tempResult		= result*5 + (*key)[(long) i];
...
typedef struct IntHasher: MyHasherModel<int>
{
	public:
		int operator() (int const &iValue) const
		{
			int			iRealValue;
			...
			// iRealValue must be positive

			return GENERIC_intHASH(iRealValue);
		}

} IntHasher_t, *IntHasher_p;

typedef struct IntCmp
{
...
typedef struct UnsignedLongHasher: MyHasherModel<unsigned long>
{
	public:
		int operator() (unsigned long const &ulValue) const
		{
			// GENERIC_HASH is defined in my_hash_map.H
			// iRealValue must be positive

			return GENERIC_intHASH(ulValue);
		}

} UnsignedLongHasher_t, *UnsignedLongHasher_p;

typedef struct UnsignedLongCmp
{
	bool operator()(const unsigned long x, const unsigned long y) const
	{
		return (x == y);
	}
} UnsignedLongCmp_t, *UnsignedLongCmp_p;
...
} StringCmp_t, *StringCmp_p;

typedef struct StringHasher: MyHasherModel<char *>
{
	public:
		int operator() (char * const &pKey) const
		{
			int i, tempResult, result = 0;
			const int length = strlen (pKey);
			for(i = 0; i < length ; i++)
			{
				tempResult		= result * 5 + pKey [(long) i];
...
} StringHasher_t, *StringHasher_p;

typedef struct CharsHasher: MyHasherModel<char *>
{
	public:
...
} CharsHasher_t, *CharsHasher_p;

typedef struct CharsCmp
{
	bool operator()(const char *x, const char *y) const
	{
		return !strcmp(x, y);
	}
} CharsCmp_t, *CharsCmp_p;

#ifdef WIN32
	typedef struct longlongCmp
	{
		bool operator ()(const __int64 x, const __int64 y) const
		{
			return (x == y);
		}
	} longlongCmp_t, *longlongCmp_p;

	typedef struct longlongHasher: MyHasherModel<__int64>
	{
		public:
			int operator() (__int64 const &llKey) const
			{
				__int64			llRealKey;
				...
				if (llKey < 0)
					...
					llRealKey		= llKey;
				// GENERIC_HASH is defined in my_hash_map.H
				// llRealKey must be positive

				return (int) (GENERIC___int64HASH(llRealKey));
			}
	} longlongHasher_t, *longlongHasher_p;
#else
	typedef struct longlongCmp
	{
	...
	typedef struct longlongHasher: MyHasherModel<long long>
	{
		public:
			int operator() (long long const &llKey) const
			{
				long long			llRealKey;
				...
				return ((int) GENERIC_longlongHASH (llRealKey));
			}
	} longlongHasher_t, *longlongHasher_p;
#endif
...
public:
  ...
  HashValue & operator[] (HashType const & key) {
    int didInsert;
    iterator i =
  ...
  iterator Search (HashType const & key) const
  {
    int hashIncrement;
  ...
  pairType * insert(HashType const & key, HashValue const & value) 
  {
    int didInsert;
    iterator i =
  ...
  iterator InsertWithoutDuplication(HashType const & key, HashValue const & value, int* didInsert) 
  {
...
template MHM_TEMPLATE
std:: ostream & my_hash_map MHM_TEMP_SPEC::operator<<
(std:: ostream &s) const
{
  int k = 0;
  while ( k < maxLength) {
