...

{
	...
	if (argc != 4)
	{
		std:: cout << "Usage: ./client <ip address> <port number> <message>"
			<< std:: endl;
	...
	if ((errInit = csClientSocket. init (SocketImpl:: DGRAM,
		2, 0, 2, 0, 2, true, (const char *) NULL)) != errNoError)
	{
	...
	std:: cout << "Message to send: " << pMessage << std:: endl;

	if ((errWrite = pSocketImpl -> writeString (pMessage,
		true, 2, 0, pIpAddress, lRemotePort)) != errNoError)
	{
		std:: cout << (const char *) errWrite << std:: endl;
		Error err = SocketErrors (__FILE__, __LINE__,
	...
	if ((errWrite = pSocketImpl -> write (&msMyStruct, sizeof (MyStruct_t),
		true, 2, 0, pIpAddress, lRemotePort)) != errNoError)
	{
		std:: cout << (const char *) errWrite << std:: endl;
		Error err = SocketErrors (__FILE__, __LINE__,
